...
// Below an hour the display carries seconds and has to keep up with them; above
// it the minute is the smallest thing shown. Five seconds costs about 2.6% of
// the time in panel refreshes and keeps the low-power clock, while landing every
// value on a five or a zero.
constexpr int kFineTickSeconds = 5;
}  // namespace
...
int countdownShownRemaining(const int seconds) {
  const int total = std::max(0, seconds);
  if (total >= kSecondsPerHour) return total;
  return (total + kFineTickSeconds - 1) / kFineTickSeconds * kFineTickSeconds;
}

int countdownShownElapsed(const int seconds) {
  const int total = std::max(0, seconds);
  if (total >= kSecondsPerHour) return total;
  return total / kFineTickSeconds * kFineTickSeconds;
}

void formatCountdownSeconds(const int shownSeconds, char* buf, const size_t len) {
  const int total = std::max(0, shownSeconds);
...
int countdownDisplayTick(const int shownSeconds) {
  const int total = std::max(0, shownSeconds);
