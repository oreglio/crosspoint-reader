...
// Repaints the reading page behind a modal (dictionary popup, quick-toggle
// drawer) without going through the reader's normal render path. Two render
// passes bracket the prewarm scope so the glyphs the page needs are resident
// before the visible pass draws them.
...
bool EpubReaderActivity::handleShortcutAction(const uint8_t rawAction) {
  ...
  switch (action) {
    ...
    case CrossPointSettings::SHORT_PWRBTN::SHORTCUT_LIBRARY:
      executeReaderQuickAction(CrossPointSettings::LONG_MENU_LIBRARY);
      return true;
