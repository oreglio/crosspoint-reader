...
void LibraryListActivity::drawRows() {
  ...
  for (int entry = topIndex; entry < count; entry++) {
    ...
    // Books with no author group under one heading of their own rather than
    // running on unlabelled: the index already files them after every named
    // author, in both directions, so the heading lands once at the end.
    const bool startsGroup = grouped && (drawn == 0 || author != previousAuthor);
    ...
    if (startsGroup) {
      ...
      // the word itself would move these books from I to U with the interface
      // language, and would merge them with a real author called "Unknown".
      std::string inverted = author.empty() ? std::string(tr(STR_LIBRARY_UNKNOWN_AUTHOR)) : author;
      if (!author.empty()) {
