...
// UiListActivity variant for screens with a tab band above the list (the
// Library shelf). Navigation is a ring: position 0 is the tab bar, 1..N are
...
// tab screens differ on press-vs-release and what Back does) plus what
// activating a row or tapping a tab means.
//
// Ported from upstream (crosspoint feat/library-view), with two fork
// extensions the shelf needs: the wantsTouchLongPress constructor flag is
// forwarded (upstream's tab screens never long-press), and a tab long-press
// dispatches through onTabLongPress() instead of being dropped.
class UiTabListActivity : public UiListActivity {
 ...
 protected:
  ...
  // wantsTouchLongPress (tab hits must also carry InputLongPress).
  virtual void onTabLongPress(int index) {}
  ...
  // Tab screens disagree on press-vs-release and Back semantics, so there is
  // no shared default.
  ...
  int ringPos() const;
  // ACTION_ROW lands as ring = row + 1, then long-press/activate.
