...
def emit_fui(name, upright, px, parts):
    """Stored UPRIGHT: the FreeInkUI bitmap path draws with no blit rotation,
    so these are the variants for fui components (list rows, tab bars)."""
    data, stride = pack(upright, px)
    art = '\n'.join('// ' + ''.join('#' if c else '.' for c in row) for row in upright)
    hexes = ', '.join(f'0x{b:02X}' for b in data)
    parts.append(
...
def main():
    ...
    for px in STAR_SIZES:
        star = rasterize_star(px)
        emit(f'icon_star_{px}', star, px, parts)
        emit_fui(f'icon_star_{px}_fui', star, px, parts)
