...
def emit_header(path, ident, data, *, original_len=None):
    ...
    it is raw bytes (NAMESize). Always emits a NAMEETag constant, already
    quoted, so the server can compare If-None-Match without building a String
    per request."""
...
def emit_gzip(path, ident, text):
    # mtime=0 keeps the gzip bytes reproducible: the header timestamp would
    # otherwise change every build, and with it the ETag emit_header derives
