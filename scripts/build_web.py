...
def emit_header(path, ident, data, *, original_len=None, etag=None):
    ...
    it is raw bytes (NAMESize). When etag is given, also emit a NAMEEtag
    constant so the server can answer If-None-Match with a 304."""
...
def emit_gzip(path, ident, text, *, etag=False):
    gz = gzip.compress(text.encode("utf-8"), compresslevel=9)
    # Etag is derived from the gzipped bytes actually sent over the wire, truncated
    # to 16 hex chars (64 bits) - plenty to detect a content change, not a security use.
    page_etag = hashlib.sha1(gz).hexdigest()[:16] if etag else None
    emit_header(path, ident, gz, original_len=len(text), etag=page_etag)
...
for slug, (ident, title, active, head_extra) in PAGES.items():
    ...
    orig, comp = emit_gzip(os.path.join(OUT, f"{ident}.generated.h"), ident, html, etag=True)
