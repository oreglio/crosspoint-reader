...
def write_version_header(project_dir, version_string):
    """Put the version in a header instead of on every compiler command line.

    As a -D it reached all ~1500 ESP-IDF translation units, so embedding the commit
    hash meant a full rebuild — two and a half minutes — every time HEAD moved. Only
    nine files actually read the version, and they all include AppVersion.h, so a
    ...
    body = (
        '// Generated by scripts/git_branch.py. Do not edit; not tracked in git.\n'
        '#pragma once\n'
        f'#define CROSSINK_VERSION_GENERATED "{version_string}"\n'
    )
    try:
