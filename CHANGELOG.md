...
- A `Library` shelf on the home screen, listing every book on the card in one place regardless of the folders they sit in. Rows carry the author beneath the title, and Left/Right turn pages while Up/Down move the selection. Sort by title, author or recently added from the strip above the list. The `Titles` tab carries its direction as a small triangle: hold `OK` on the focused tab — or flip the `A-Z | Z-A` line above the letter grid — to switch between A-Z and Z-A. Long titles sit on one line, truncated with an ellipsis, so more books fit per screen; the full title is always on the book's `Details` page.
