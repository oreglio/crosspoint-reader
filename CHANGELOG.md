...
- Search the Library by title or author from the `Search` entry in the sort strip. Typing the start of each word is enough — `dar mat` finds `Dark Matter` — which matters on a screen where every keypress redraws the whole page.
- Jump to a letter with the `A-Z` grid, opened with `OK` on an alphabetical sort. Sorted by author you choose whether the letter means the first name or the last, because no rule can tell `Qiu Xiaolong` from `Sally Rooney`.
