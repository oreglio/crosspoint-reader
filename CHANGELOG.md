...
- Mark a book as a favorite from its long-press menu. Favorite rows carry a star in place of the book icon, the star tab leads the strip and shows favorites only — with its own sort menu on a long press (`Recent`, `Titles A-Z`, `Titles Z-A`, `Author`) — and the flag survives index rebuilds and moves to another folder, because it is stored in its own file keyed by file identity rather than in the rebuildable index. The shelf remembers its view, sort and direction for the whole power-on session, and backing out lands Home on the `Library` entry.
